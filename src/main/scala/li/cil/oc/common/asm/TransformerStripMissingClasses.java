  ...
  public static byte[] transform(LaunchClassLoader loader, String name, byte[] classBytes) {
    ...
    for (String interfaceName : classNode.interfaces) {
      if (!ASMHelpers.classExists(loader, interfaceName)) {
        OpenComputers.log().trace("Stripping interface {} from class {} because it is missing.", interfaceName, name);
    ...
    for (InnerClassNode innerClass : classNode.innerClasses) {
      if (innerClass.outerName != null && !ASMHelpers.classExists(loader, innerClass.outerName)) {
        OpenComputers.log().trace("Stripping inner class {} from class {} because its type {} is missing.", innerClass.name, name, innerClass.outerName);
    ...
    for (MethodNode method : classNode.methods) {
      ...
      if (!missing.isEmpty()) {
        OpenComputers.log().trace("Stripping method {} from class {} because missing types in signature: {}",
